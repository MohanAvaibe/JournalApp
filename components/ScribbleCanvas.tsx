...
const ScribbleCanvas: React.FC = () => {
  ...
  function handleEnd() {
    ...
    let finishedPath = currentPathRef.current;
    if (finishedPath.pts.length === 1) {
      // Single tap, draw the dot
      const pt = finishedPath.pts[0];
      const radius = finishedPath.strokeWidth / 2;
      const dotPath = Skia.Path.Make();
      dotPath.addCircle(pt.x, pt.y, radius);
      finishedPath = { ...finishedPath, path: dotPath };
    }
  ...
  const tap = Gesture.Tap().onStart(e => runOnJS(handleTap)(e.x, e.y));
    ...
    setPages(prevPages => {
      ...
      for (const group of groups) {
        ...
        group.forEach(stroke => {
          const newPath = stroke.path.copy();
